...
## 三-bis、彩蛋：隐藏开发者测试面板 func 0xaa5848

追踪 `onReward Cheat Triggered!` 的第 3 个加载点 0xaa62f4 时，发现 func **0xaa5848** 是一个**隐藏的开发者/测试面板**，含：
- `CrackTest`（ref 12105）/ `AdsTest`（ref 6280）/ `ShellTest`（ref 15586）/ `JsTest`（ref 28346）
- `输入要运行的JS代码`（ref 20368，可运行任意 JS）/ `输入类名` / `输入命令行` / `输入文件名`
- `icudtl.dat`、`AssetsTest`

这是一个开发期遗留的调试后门（测广告/测破解/跑 shell/跑 JS）。**本身不是 patch 目标**，但说明：(1) app 内部有现成的"测广告"入口可参考其 ad-decision 逻辑；(2) 若主控能在运行时触发该面板（如某隐藏手势/输入），可直接调试广告与 VIP 行为。反作弊 `onReward Cheat Triggered!` 在此只是测试用日志串，**真正的作弊判定逻辑在 0x8758bc 奖励分支内**，需进一步反汇编该分支定位比较字节。
