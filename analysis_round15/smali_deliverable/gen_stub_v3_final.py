#!/usr/bin/env python3
"""Generate AdNoOpPluginV3.smali (EventChannel reward stub).

This is imported into test1 as a collaboration artifact from the converged
test3 route. It extracts exact mixed-script Flutter API descriptors from an
apktool tree, then regenerates the V3 stub without hand-typing the obfuscated
class names.
"""
...
PKG = chr(0x0E2A) + chr(0x06E5) + chr(0x06E0) + chr(0x06E6) + chr(0x06E1)
BASE = os.path.join(APK, "smali_classes2", PKG)


def read_file(basename: str) -> str:
    return open(os.path.join(BASE, basename), encoding="utf-8").read()


def class_desc(text: str) -> str:
    return re.search(r"\.class[^\n]*?(L[^;]+;)", text).group(1)


v1 = open(
    os.path.join(APK, "smali_classes2/com/gentle/ppcat/AdNoOpPlugin.smali"),
    encoding="utf-8",
).read()
...
onep = [(name, params) for name, params in am if "$" in params and params.count("L") == 1]
attach = max(onep, key=lambda item: len(item[0]))[0]
detach = min(onep, key=lambda item: len(item[0]))[0]
binding = onep[0][1]

twop = [(name, params) for name, params in am if params.count("L") == 2]
onmethod = twop[0][0]
a, b = twop[0][1].split(";L", 1)
mcall = a + ";"
result = "L" + b

gbm = re.search(r"invoke-virtual \{p1\}, (L[^)]+?;)->(\S+)\(\)(L[^;]+;)", v1)
binding = gbm.group(1)
getbm = gbm.group(2)
bmess = gbm.group(3)
...
setmch = re.search(
    r"invoke-virtual \{v1, p0\}, (L[^)]+?;)->(\S+)\((L[^)]+?;)\)V", v1
).group(2)
result_success = re.search(
    r"invoke-interface \{p2, v0\}, (L[^)]+?;)->(\S+)\(Ljava/lang/Object;\)V",
    v1,
).group(2)

ev = read_file("สۥ۟۟ۡ.smali")
evchan = class_desc(ev)
ev_ctor = re.search(r"\.method public constructor <init>\((L[^)]+?;)Ljava/lang/String;(L[^)]+?;)\)V", ev)
codec_base = ev_ctor.group(2)
bmess2 = ev_ctor.group(1)
assert bmess2 == bmess

setsh_m = re.search(r"\.method public (\S+)\((L[^)]+?\$สۥۣ۟۟;)\)V", ev)
setsh = setsh_m.group(1)
sh_iface = setsh_m.group(2)

codec = class_desc(read_file("สۥ۟۟.smali"))
sh = read_file("สۥ۟۟ۡ$สۥۣ۟۟.smali")
...
sink_success = re.search(
    r"\.method public abstract (\S+)\(Ljava/lang/Object;\)V",
    read_file("สۥ۟۟ۡ$สۥ۟۟ۤ.smali"),
).group(1)

assert ".method public constructor <init>()V" in read_file("สۥ۟۟.smali")
...
parts = [
    f""".class public Lcom/gentle/ppcat/AdNoOpPluginV3;
...
output = os.path.join(APK, "smali_classes2/com/gentle/ppcat/AdNoOpPluginV3.smali")
open(output, "w", encoding="utf-8").write("".join(parts))
print("wrote", output)
