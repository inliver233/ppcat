...
P_CAND1 = [(0xbafd70, bytes.fromhex("e00316aa")),(0xbafd74, bytes.fromhex("ef031daa")),(0xbafd78, bytes.fromhex("fd79c1a8")),(0xbafd7c, bytes.fromhex("c0035fd6"))]

P_OVERLAY_NULL = [(0xbd2e30, bytes.fromhex("e00316aa")),(0xbd2e34, bytes.fromhex("ef031daa")),(0xbd2e38, bytes.fromhex("fd79c1a8")),(0xbd2e3c, bytes.fromhex("c0035fd6"))]  # 0xbd2e1c home-overlay orchestrator entry-null (test 同源 hypothesis: cat block=overlay?)
P_OVERLAY3540_NULL = [(0xbd3554, bytes.fromhex("e00316aa")),(0xbd3558, bytes.fromhex("ef031daa")),(0xbd355c, bytes.fromhex("fd79c1a8")),(0xbd3560, bytes.fromhex("c0035fd6"))]  # 0xbd3540 overlay+banner dual builder entry-null

P_READER_NULL = [(0xb799cc, bytes.fromhex("e00316aa")),(0xb799d0, bytes.fromhex("ef031daa")),(0xb799d4, bytes.fromhex("fd79c1a8")),(0xb799d8, bytes.fromhex("c0035fd6"))]  # 诊断: reader build 0xb799b8 entry-null
...
CONFIGS = {
    ...
    "testC_cand1": P_TAMPER + P_FAULT_GATE + P_DIALOG_NOP + P_FAULT_BODY + P_CAND1,
    "testC_ovnull": P_TAMPER + P_FAULT_GATE + P_DIALOG_NOP + P_FAULT_BODY + P_OVERLAY_NULL,  # 故障根治+home-overlay编排器0xbd2e1c entry-null(测同源)
    "testC_ov3540": P_TAMPER + P_FAULT_GATE + P_DIALOG_NOP + P_FAULT_BODY + P_OVERLAY3540_NULL,  # 故障根治+overlay/banner双源0xbd3540 entry-null
    "testC_ovboth": P_TAMPER + P_FAULT_GATE + P_DIALOG_NOP + P_FAULT_BODY + P_OVERLAY_NULL + P_OVERLAY3540_NULL,  # 故障根治+0xbd2e1c+0xbd3540 双overlay entry-null
    "testC_ov_cand1": P_TAMPER + P_FAULT_GATE + P_DIALOG_NOP + P_FAULT_BODY + P_OVERLAY_NULL + P_CAND1,  # overlay根治+reader候选0xbafd68 entry-null(定位reader build)
    "testC_reader_null": P_TAMPER + P_FAULT_GATE + P_DIALOG_NOP + P_FAULT_BODY + P_READER_NULL,  # 诊断: 故障根治+reader build null
